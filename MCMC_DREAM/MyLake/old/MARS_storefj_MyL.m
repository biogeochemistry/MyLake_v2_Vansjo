...
clear all;
%path(path,'C:\OneDrive\SOFTWARE\Mylake_v12_package\air_sea') %path for air-sea toolbox
%path(path,'C:\OneDrive\SOFTWARE\Mylake_v12_package\v12') %path for MyLake model code
global ies80 Eevapor;
test_time=0;
Eevapor=0;

load obs\store_obs\vansjoice.dat
load obs\store_obs\vansjotemp.dat 

load obs\store_obs\Cha.dat
load obs\store_obs\PO4.dat
load obs\store_obs\Part.dat
load obs\store_obs\TOTP.dat
...
inputfile='store_INCAP_input.txt'
initfile='store_init.txt' % Tuomo's work
parafile='store_para.txt'
...
% thermocline depth
zt = MixStat(7,:); %RMC Feb 18th 2014 changed 10 to 7

%% RMC figure

%% Total P
tlims=[datenum([2002,4,1]) datenum([2010,10,12])];

zinx=find(zz<4); %depth layer considered

temp=(Pzt(zinx,:)+PPzt(zinx,:)+DOPzt(zinx,:)+Chlzt(zinx,:))'; % total P
tp_datum=mean(temp,2); % this is the mean of  depth-averaged time series

temp=Chlzt(zinx,:)'; % this is the mean of  depth-averaged time series
chl_datum=mean(temp,2);
...
subplot(4,1,1)% (m,n,p) matrix of plots, current plot is p
    ...
    ylabel('P (ug L-1)','FontWeight','bold','FontSize',12);
    datetick
...
%     x = POP_datum(start_x:end,1);
%     y1 = POP_datum(start_x:end,4);
%     y2 = POP_datum(start_x:end,3);
%     fill([x;flipud(x)],[y1;flipud(y2)],'b','FaceAlpha', 0.4, 'edgecolor','none')
...
for i=1:1    %dummy to hide TSA's figure
%% Tuomo figure 
...
%          TP_Chla_Mod=NaN*ones(length(Cha),2); 
%         for i=1:length(Cha)
%             inx=find(Cha(i,1)==monthly_qual(:,4));
%             if(isempty(inx)==0)
%                 TOTP(i,2)=monthly_qual(inx,5); %1) TP
%                 Cha(i,2)=monthly_qual(inx,6); %2) Chla
%             end
%         end
%  
...
% clf
% for i=1:length(Cha)
% inx=find(Cha(:,1)==monthly_qual(:,4));
%  if(isempty(inx)==0)
%  subplot(221)  
%  plot(TOTP(:,2),monthly_qual(inx,5),'b.')
%  hold on
%  subplot(222)  
%  plot(Cha(:,2),monthly_qual(inx,6),'b.')
%  hold on
%  end
% end
% subplot(221)
...
% subplot(223)
% for i=1:length(Part)
% inx=find(Part(i,1)==monthly_qual(:,4));
%  if(isempty(inx)==0)
%  loglog(1e-3*Part(i,2),monthly_qual(inx,7),'b.') %mg/L -> g/L
%  hold on
%  end
% end
% axis('square')
...
% subplot(224)
% for i=1:length(PO4)
% inx=find(PO4(i,1)==monthly_qual(:,4));
%  if(isempty(inx)==0)
%  plot(PO4(i,2),monthly_qual(inx,9),'b.') %mg/m3
%  hold on
%  end
% end
% axis('square')
...
%  
%  datetick('x','mmm');
%  H=legend('Inflow of P', 'Outflow of P', 'Sedimentation of P', 'nge in lake P', 'P-Balance');
%  set(H,'fontsize',8);
%  grid on;
%   ylabel('kg')
%   
...
%  
%  subplot(312)
%  plot(monthly_qual(:,4),monthly_qual(:,10),'.-')
%  hold on
%  plot(Part(:,1),Part(:,2),'r+') % removed mg/L -> g/L
%  set(gca,'ylim',[0 40]);
%  ylabel('kg/m^3','fontsize',9)
%  xlabel('year','fontsize',9)
%  title('Part (monthly mean, 0-4m)','fontweight','bold')
%  datetick('x','yy')
%  grid on
%  set(gca,'fontsize',9)
%  set(gca,'xlim',tlims)
