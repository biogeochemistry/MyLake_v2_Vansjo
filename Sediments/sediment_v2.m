...
% Estimate flux
  ...
  sediment_bioirrigation_fluxes.DOM2 = - top_sediment_rate_to_flux(bioirrigation(DOM2(:,end), alfax, fi),30973.762,dx,m);
...
% NOTE: the rates are the same as in sediments except microbial which are factorized due to lower concertation of bacteria in WC then in sediments. Units are per "year" due to time step is in year units too;
    ...
    dcdt(:,4)  = - 0.8*(Cx1*R2a+Cx1*R2b+Cx1*R2c+Cx1*R2d)+ R9 + bioirrigation(NO3, alfax, fi); % NO3
    dcdt(:,5)  = -4 * (Cx1*R3a + Cx2*R3b+Cx1*R3c + Cx2*R3d) - 2*R7 + R8 - R16a + R16b; % FeOH3
    ...
    dcdt(:,9)  = -4*(Cx1*R4a + Cx2*R4b + Cx1*R4c + Cx2*R4d) + R12 - R17a + R17b; % FeOOH
