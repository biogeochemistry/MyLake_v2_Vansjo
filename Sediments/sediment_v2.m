function [ sediment_bioirrigation_fluxes, sediment_transport_fluxes, sediment_concentrations, sediment_additional_results] = sediment_v2(sediment_concentrations, sediment_params, sediment_matrix_templates, sediment_bc)
  ...
  for i=2:m
    ...
    % =======================================================================================================
      ...
      POP(:,i) = pde_solver_neumann(POP_AL, POP_AR, POP(:,i), sediment_bc.POP_fx, sediment_params.solid_flux_coef);
      POC(:,i) = pde_solver_neumann(POC_AL, POC_AR, POC(:,i), sediment_bc.POC_fx, sediment_params.solid_flux_coef);
      NO3(:,i) = pde_solver_dirichlet(NO3_AL, NO3_AR, NO3(:,i), sediment_bc.NO3_c);
      FeOH3(:,i) = pde_solver_neumann(FeOH3_AL, FeOH3_AR, FeOH3(:,i), sediment_bc.FeOH3_fx, sediment_params.solid_flux_coef);
      SO4(:,i) = pde_solver_dirichlet(SO4_AL, SO4_AR, SO4(:,i), sediment_bc.SO4_c);
      ...
      FeOOH(:,i) = pde_solver_neumann(FeOOH_AL, FeOOH_AR, FeOOH(:,i), sediment_bc.FeOOH_fx, sediment_params.solid_flux_coef);
      ...
      FeS(:,i) = pde_solver_neumann(FeS_AL, FeS_AR, FeS(:,i), sediment_bc.FeS_fx, sediment_params.solid_flux_coef);
      ...
      S8(:,i) = pde_solver_neumann(S8_AL, S8_AR, S8(:,i), sediment_bc.S8_fx, sediment_params.solid_flux_coef);
      FeS2(:,i) = pde_solver_neumann(FeS2_AL, FeS2_AR, FeS2(:,i), sediment_bc.FeS2_fx, sediment_params.solid_flux_coef);
      AlOH3(:,i) = pde_solver_neumann(AlOH3_AL, AlOH3_AR, AlOH3(:,i), sediment_bc.AlOH3_fx, sediment_params.solid_flux_coef);
      PO4adsa(:,i) = pde_solver_neumann(PO4adsa_AL, PO4adsa_AR, PO4adsa(:,i), sediment_bc.PO4adsa_fx, sediment_params.solid_flux_coef);
      PO4adsb(:,i) = pde_solver_neumann(PO4adsb_AL, PO4adsb_AR, PO4adsb(:,i), sediment_bc.PO4adsb_fx, sediment_params.solid_flux_coef);
      Ca2(:,i) = pde_solver_dirichlet(Ca2_AL, Ca2_AR, Ca2(:,i), sediment_bc.Ca2_c);
      Ca3PO42(:,i) = pde_solver_neumann(Ca3PO42_AL, Ca3PO42_AR, Ca3PO42(:,i), sediment_bc.Ca3PO42_fx, sediment_params.solid_flux_coef);
      OMS(:,i) = pde_solver_neumann(OMS_AL, OMS_AR, OMS(:,i), sediment_bc.OMS_fx, sediment_params.solid_flux_coef);
      CaCO3(:,i) = pde_solver_neumann(CaCO3_AL, CaCO3_AR, CaCO3(:,i), sediment_bc.CaCO3_fx, sediment_params.solid_flux_coef);
      ...
      Chl(:,i) = pde_solver_neumann(Chl_AL, Chl_AR, Chl(:,i), sediment_bc.Chl_fx, sediment_params.solid_flux_coef);
      CH4aq(:,i) = pde_solver_dirichlet(CH4aq_AL, CH4aq_AR, CH4aq(:,i), sediment_bc.CH4aq_c);
      CH4g(:,i) = pde_solver_dirichlet(CH4g_AL, CH4g_AR, CH4g(:,i), sediment_bc.CH4g_fx);
      FeCO3(:,i) = pde_solver_dirichlet(FeCO3_AL, FeCO3_AR, FeCO3(:,i), sediment_bc.FeCO3_fx);
      Fe3PO42(:,i) = pde_solver_dirichlet(Fe3PO42_AL, Fe3PO42_AR, Fe3PO42(:,i), sediment_bc.Fe3PO42_fx);
    ...
    sediment_transport_fluxes.Ca2     = convert_flux_umol_per_cm2_y_to_mg_per_m2_d(mean(sediment_transport_fluxes.Ca2), 80156);
  ...
  phi = sediment_params.phi;
    ...
    elseif algorithm == 2
      for i=1:size(H3O,1)
        ...
        sediment_params.aq_system.fe2.conc = 1e-3*(Fe2(i)) * 0.8; % 0.5 accounts for sorptio
...
% NOTE: the rates are the same as in sediments. Units are per "year" due to time step is in year units too;
    ...
    dcdt(:,8)  = - bioirrigation(Fe2, alfax, phi) +  4*(Cx2*R3a+Cx3*R3b+Cx1*R3f) .* F + 4* (Cx2*R3c + Cx3*R3d) + 4*(Cx2*R4a + Cx3*R4b+ Cx1*R4f) .* F + 4 * (Cx2*R4c + Cx3*R4d) + 2*R12.*F - R13 + R27b.*F - R27a.*F - R29a.*F + R29b.*F -3*R33a + 3*R33b; % Fe2(aq)
    dcdt(:,9)  = -4 * (Cx2*R4a_Fe + Cx3*R4b_Fe + Cx1*R4f_Fe) - 4*(Cx2*R4c_Fe + Cx3*R4d_Fe)./F + R25 - R32a + R23./F; % FeOOH(solid)
    dcdt(:,10) = - bioirrigation(H2S, alfax, phi) - H2S./(Sum_H2S+1e-16).* (R11 + R12.*F + R27b.*F + R27a.*F + R26 + Cx2*R21a + Cx3*R21b + Cx2*R21c + Cx3*R21d + Cx1*R21f); % H2S(aq)
    dcdt(:,11) = - bioirrigation(HS, alfax, phi) +  0.5*(Cx2*R5a + Cx3*R5b + Cx1*R5f) .* F + 0.5 * (Cx2*R5c + Cx3*R5d)  + R16 - HS./(Sum_H2S+1e-16).* (R11 + R12.*F + R27b.*F + R27a.*F + R26 + Cx2*R21a + Cx3*R21b + Cx2*R21c + Cx3*R21d + Cx1*R21f) ; % HS(aq)
    dcdt(:,12) =  - R24 - 4*R25 -R26./F + R27a - R27b ; % FeS(solid)
    dcdt(:,13) = - R24.*F - R22a + R12.*F + R22b.*F; % S0(aq)
    dcdt(:,14) = - bioirrigation(PO4, alfax, phi) +  (Pz2 * Ra + Pz3 * Rb + Pz1 * Rf) .* F + (Pz2 * Rc + Pz3 * Rd) + 4 * (Cx2*R3a_P + Cx3*R3b_P + Cx1*R3f_P) .*F + 4 * (Cx2*R3c_P + Cx3*R3d_P)  + 4 * (Cx2*R4a_P + Cx3*R4b_P + Cx1*R4f_P).*F  + 4 * (Cx2*R4c_P + Cx3*R4d_P) - R31a.*F - R32a.*F - R35a.*F + R35b.*F - 2 * R34a + 2 * R34b -2*R33a + 2*R33b; % PO4(aq)
    ...
    dcdt(:,20) = - bioirrigation(Ca2, alfax, phi) -3*R34a + 3*R34b - R28a.*F + R28b.*F; % Ca2(aq)
    dcdt(:,21) = R34a./F - R34b./F; % Ca3PO42(s)
    dcdt(:,22) = (Cx2*R21a + Cx3*R21b + Cx2*R21c + Cx3*R21d + Cx1*R21f)./F; % OMS(s)
    dcdt(:,23) = R28a - R28b; % CaCO3(s)
    ...
    dcdt(:,25) = - bioirrigation(CO3, alfax, phi) - R28a.*F + R28b.*F - R29a.*F + R29b.*F; % CO3(aq)
