function [ sediment_bioirrigation_fluxes, sediment_SWI_fluxes, sediment_integrated_over_depth_fluxes, sediment_concentrations, xz, R_values] = sediment(sediment_concentrations, sediment_params, sediment_matrix_templates, species_sediment, sediment_bc)
  % SEDIMENTS This function models the chemical process in the sediment

  % global k_OM k_OMb Km_O2 Km_NO3 Km_FeOH3 Km_FeOOH Km_SO4 Km_oxao Km_amao Kin_O2 Kin_NO3  Kin_FeOH3 Kin_FeOOH k_amox k_Feox k_Sdis k_Spre k_FeS2pre k_pdesorb_c k_pdesorb_a k_pdesorb_b k_alum k_rhom   k_tS_Fe Ks_FeS k_Fe_dis k_Fe_pre k_apa  kapa k_oms k_tsox k_FeSpre f_pfe accel Cx1 Ny1 Pz1 Cx2 Ny2 Pz2
  ...
  F_OM_top = ones(1,m) * 1/F .* sediment_bc('OM1_fx'); % Flux on the top for solid species

  % for Organic matter B:
  % Flux of OM2 from Lake
  F_OMb_top = ones(1,m) * 1/F .* sediment_bc('OM2_fx'); % Flux on the top for solid species
  ...
  % for Fe(OH)3
  F_FeOH3_top = ones(1,m) * 1/F .* sediment_bc('FeOH3_fx'); % 14.7; %from Canavan et al AML
  ...
  % for FeOOH
  F_FeOOH_top = ones(1,m) * 1/F .* sediment_bc('FeOOH_fx'); % 0; %from Canavan et al AML


  % for FeS
  F_FeS_top = ones(1,m) * 1/F .* sediment_bc('FeS_fx'); % 0; % Flux for solid species
  ...
  % for S8
  F_S8_top = ones(1,m) * 1/F .* sediment_bc('S8_fx'); % 0; %from Canavan et al AML

  % for FeS2
  F_FeS2_top = ones(1,m) * 1/F .* sediment_bc('FeS2_fx'); % 0; % Flux for solid species

  % for AlOH3
  F_AlOH3_top = ones(1,m) * 1/F .* sediment_bc('AlOH3_fx'); % 0 % Flux for solid species

  % for PO4adsa
  F_PO4adsa_top = ones(1,m) * 1/F .* sediment_bc('PO4adsa_fx'); % 0; % Flux for solid species

  % for PO4adsb
  F_PO4adsb_top = ones(1,m) * 1/F .* sediment_bc('PO4adsb_fx'); % 0; % Flux for solid species
  ...
  % for Ca3PO42
  F_Ca3PO42_top = ones(1,m) * 1/F .* sediment_bc('Ca3PO42_fx'); % 0; % Flux for solid species

  % for OMS
  F_OMS_top = ones(1,m) .* 1/F .* sediment_bc('OMS_fx'); % 0; % Flux for solid species
  ...
  for i=2:m
    ...
    % In R_eq_non_lin should be all variables due to it is outside matrix multiplication
    Ox_dRbio(:,i) = bioirrigation(Ox(:,i-1), alfax, fi);
    R_eq_Ox_non_lin(:,i) = Ox_dRbio(:,i);
    ...
    NO3_dRbio(:,i) = bioirrigation(NO3(:,i-1), alfax, fi);
    R_eq_NO3_non_lin(:,i) = R9 + NO3_dRbio(:,i);
    ...
    Fe2_dRbio(:,i) = bioirrigation(Fe2(:,i-1), alfax, fi);
    R_eq_Fe2_non_lin(:,i) = 4*R3*F + 4*R4*F + 2*R7 + R14b - R14a + Fe2_dRbio(:,i); %NOTE: concentration of Iron in Sat_FeS can be improved by implementing and adding in R_eq_matrix
    ...
    R19 = (R19 >= 0) .* R19;
    PO4_dRbio(:,i) = bioirrigation(PO4(:,i-1), alfax, fi);
    R_eq_PO4_non_lin(:,i) = (P_index1 * Ra + P_index2 * Rb)*F + R16b + R17b - 2 * R19 + PO4_dRbio(:,i);
    ...
    R5b = Rc2 .* f_SO4 .* SO4(:,i-1) ;             %OM oxidation by SO4
    Rb = R1b+R2b+R3b+R4b+R5b;                      % degradation of OM2
    NH4_dRbio(:,i-1) = bioirrigation(NH4(:,i-1), alfax, fi);
    R_eq_NH4_non_lin(:,i) = (N_index1 * Ra + N_index2 * Rb) * F + NH4_dRbio(:,i-1);
    ...
    % Oxygen consumption in sediments.
    % R_ox_cons(:,i-1) = R1_graphs(:,i-1) + 2*R6_graphs(:,i-1) + 0.25
    R8 = k_Feox .* Fe2(:,i-1) .* Ox(:,i-1);
    R9 = k_amox .* Ox(:,i-1) ./ (Km_oxao + Ox(:,i-1)) .* (NH4(:,i-1) ./ (Km_amao + NH4(:,i-1)));
    R12 = k_rhom  .* FeS(:,i-1) .* Ox(:,i-1);
    Rc1 = k_OM * OM(:,i-1) .* Ox(:,i-1);
    Rc2 = k_OMb * OMb(:,i-1) .* Ox(:,i-1);
    R1a = Rc1 .* f_O2 * accel .* Ox(:,i-1);
    R1b = Rc2 .* f_O2 * accel .* Ox(:,i-1);
    R1 = R1a+R1b;
    R_ox_consump(:,i) = 0.25 * R8  + 2 * R9  + R1 * F + 3 * R12;
...
% Estimate flux
  ...
  sediment_SWI_fluxes = {...
    Ox_flux,                   'O2 flux';
    sediment_bc('OM1_fx'),     'OM1 flux';
    sediment_bc('OM2_fx'),     'OM2 flux';
    PO4_flux,                  'PO4 flux';
    NO3_flux,                  'NO3 flux';
    sediment_bc('FeOH3_fx'),   'FeOH3 flux';
    Fe2_flux,                  'Fe2+ flux';
    NH4_flux,                  'NH4+ flux';
    sediment_bc('AlOH3_fx'),   'AlOH3 flux';
    sediment_bc('PO4adsa_fx'), 'PO4adsa flux';
  };


  % Flux due to bioirrigation:
  sediment_bioirrigation_fluxes = {...
    top_sediment_rate_to_flux(Ox_dRbio,31998,dx,m),         'Oxygen bioirrigation flux';
    top_sediment_rate_to_flux(PO4_dRbio,94971,dx,m),        'PO4 bioirrigation flux';
    top_sediment_rate_to_flux(Fe2_dRbio,55845,dx,m),        'Fe2+ bioirrigation flux';
    top_sediment_rate_to_flux(NO3_dRbio,62004,dx,m),        'NO3- bioirrigation flux';
    top_sediment_rate_to_flux(NH4_dRbio,18038,dx,m),        'NH4+ bioirrigation flux';
  };

  % Update concentration if it has resuspension flux and coupled to MyLake:
  % Ox(:,end) = update_concentration_due_to_resuspension_flux(Ox(:,end), D_O2, fi, t(end) , dx);
  % PO4(:,end) = update_concentration_due_to_resuspension_flux(PO4(:,end), D_PO4, fi, t(end) , dx);
  % Fe2(:,end) = update_concentration_due_to_resuspension_flux(Fe2(:,end), D_Fe2, fi, t(end) , dx);
  % NO3(:,end) = update_concentration_due_to_resuspension_flux(NO3(:,end), D_NO3, fi, t(end) , dx);
  % NH4(:,end) = update_concentration_due_to_resuspension_flux(NH4(:,end), D_NH4, fi, t(end) , dx);
  ...
  sediment_integrated_over_depth_fluxes = {...
    top_sediment_rate_to_flux(R_ox_consump, 31998,dx,m),   'Ox integrated over depth flux to sediments';
  };
    % Debugging purposes
  if debugg
    if any(isnan(top_sediment_rate_to_flux(Ox_dRbio,31998,dx,m))) | any(isnan(top_sediment_rate_to_flux(PO4_dRbio,94971,dx,m))) | any(isnan(top_sediment_rate_to_flux(Fe2_dRbio,55845,dx,m))) | any(isnan(top_sediment_rate_to_flux(NO3_dRbio,62004,dx,m))) | any(isnan(top_sediment_rate_to_flux(NH4_dRbio,18038,dx,m)))
      error('Breaking out of Sediments function: NaN values');
    end
...
function [C] = update_concentration_due_to_resuspension_flux(C, D, fi, dt , dx)
  % dt = 1 day of MyLake
  % flux is estimated
  
  dC = (C(2)- C(1));
  
  % Estimating the diffusion flux:
  flux = - D * fi(1) * dC / dx;
  
  % Change only of it is outward flux:
  flux = flux * ( flux > 0 );
  % Change C(2) because C(1) is boundary condition.
  C(2) = C(2) - flux *  dt *  1 / dx;
