...
function [zz,Az,Vz,tt,Qst,Kzt,Tzt,Czt,Szt,Pzt,Chlzt,PPzt,DOPzt,DOCzt,DICzt,CO2zt,O2zt,O2_sat_relt,O2_sat_abst,BODzt,Qzt_sed,lambdazt,...
        P3zt_sed,P3zt_sed_sc,His,DoF,DoM,MixStat,Wt,surfaceflux,O2fluxt,CO2_eqt,K0t,O2_eqt,K0_O2t,...
        CO2_ppmt,dO2Chlt,dO2BODt,dO2SODt,testi1t,testi2t,testi3t,...
         sediment_results] = ...
...
matsedlab_sediments_module = 1;           % MATSEDLAB sediment module
%fokema
...
if (nargin>8) %if optional command line parameter input is used 
    ...
    [In_Z,In_Az,tt,In_Tz,In_Cz,In_Sz,In_TPz,In_DOPz,In_Chlz,In_DOCz,In_DICz,In_O2z,In_TPz_sed,In_Chlz_sed,In_FIM,Ice0,Wt,Inflw,...
...
w_s = Bio_par(8);            %NOTE: settling velocity for S (m day-1)  
w_chl = Bio_par(9);          %NOTE: settling velocity for Chl a (m day-1)
...
BOD7 = Bio_par(30);             %Biological oxygen demand (mg m-3)
...
[sediments_concetrations, sediment_params, sediments_matrices_templates, species_sediments]  = sediments_init( pH );
% ==========================================
...
for i = 1:length(tt)
    ...
    [Tz,Cz,Sz,Pz,Chlz,PPz,DOPz,DOCz,DICz,O2z] = convection_v12_1a(Tz,Cz,Sz,Pz,Chlz,PPz,DOPz,DOCz,DICz,O2z,Tprof_prev,Vz,Cw,f_par,lambdaz_wtot_avg,zz,swa_b0,tracer_switch,1);
    ...
    [Tz,Cz,Sz,Pz,Chlz,PPz,DOPz,DOCz,DICz,O2z] = convection_v12_1a(Tz,Cz,Sz,Pz,Chlz,PPz,DOPz,DOCz,DICz,O2z,Tprof_prev,Vz,Cw,f_par,lambdaz_wtot_avg,zz,swa_b0,tracer_switch,0);
...
%================================    
    ...
    for k = 1:length(Tz)
        ...
        % This is where DOC is consummed and oxidized 

    dO2_BOD = DOCz .* k_BOD.*theta_b.^(Tz-20);
    %dO2_BOD = 6000 * k_BOD.*theta_b.^(Tz-20);
...
%     DICz = Fi \ DICz;
    ...
    H_netsed_org=max(0, (Vz./(-diff([Az; 0]))).*(delC_org+delC_org2)./(F_OM*Y_cp*rho_org) - (1-F_IM).*S_resusp);
    ...
    if (river_inflow_switch==1)
        ...
        if(Iflw>0)
            ...
            dummy=IOflow_v11(dz, zz, Vz, DICz, lvlD, Iflw, Iflw_DIC);
            DICz=dummy; %DIC
    ...
    if matsedlab_sediments_module == 1
        ...
        sediments_bc = wc_sediment_bc(MyLake_concentrations, MyLake_params, sediment_params);
        [sediments_fluxes, sediments_concetrations, z_matsedlab, R_values_matsedlabz] = sediments(...
            sediments_concetrations, sediment_params, sediments_matrices_templates, species_sediments, sediments_bc);

        O2z(end) = O2z(end) - sediments_fluxes{1} * dt * Az(end) / Vz (end);
        Pz(end) = Pz(end) - sediments_fluxes{4} * dt * Az(end) / Vz (end);
        deltaO2(i) = - sediments_fluxes{1} * dt * Az(end) / Vz (end);
        deltaPz(i) = - sediments_fluxes{4} * dt * Az(end) / Vz (end);
    end
    ...
    O2_matsedlabzt(:,i) = sediments_concetrations('Oxygen');
    OM_matsedlabzt(:,i) = sediments_concetrations('OM1');
    OMb_matsedlabzt(:,i) = sediments_concetrations('OM2');
    NO3_matsedlabzt(:,i) = sediments_concetrations('NO3');
    FeOH3_matsedlabzt(:,i) = sediments_concetrations('FeOH3');
    SO4_matsedlabzt(:,i) = sediments_concetrations('SO4');
    NH4_matsedlabzt(:,i) = sediments_concetrations('NH4');
    Fe2_matsedlabzt(:,i) = sediments_concetrations('Fe2');
    FeOOH_matsedlabzt(:,i) = sediments_concetrations('FeOOH');
    H2S_matsedlabzt(:,i) = sediments_concetrations('H2S');
    HS_matsedlabzt(:,i)  = sediments_concetrations('HS');
    FeS_matsedlabzt(:,i) = sediments_concetrations('FeS');
    S0_matsedlabzt(:,i)  = sediments_concetrations('S0');
    PO4_matsedlabzt(:,i) = sediments_concetrations('PO4');
    S8_matsedlabzt(:,i) = sediments_concetrations('S8');
    FeS2_matsedlabzt(:,i) = sediments_concetrations('FeS2');
    AlOH3_matsedlabzt(:,i) = sediments_concetrations('AlOH3');
    PO4adsa_matsedlabzt(:,i) = sediments_concetrations('PO4adsa');
    PO4adsb_matsedlabzt(:,i) = sediments_concetrations('PO4adsb');
    H_matsedlabzt(:,i) = sediments_concetrations('H');
    Ca2_matsedlabzt(:,i) = sediments_concetrations('Ca2');
    Ca3PO42_matsedlabzt(:,i) = sediments_concetrations('Ca3PO42');
    OMS_matsedlabzt(:,i) = sediments_concetrations('OMS');
    O2_flux_matsedlabzt(i) = sediments_fluxes{1};
    OM_flux_matsedlabzt(i) = sediments_fluxes{2};
    OM2_flux_matsedlabzt(i) = sediments_fluxes{3};
    PO4_flux_matsedlabzt(i) = sediments_fluxes{4};
    R_oxygen_matsedlabzt(:,i) = R_values_matsedlabz{1};
    R_NO3_matsedlabzt(:,i) = R_values_matsedlabz{2};
    R_FeOH3_matsedlabzt(:,i) = R_values_matsedlabz{3};
    R_FeOOH_matsedlabzt(:,i) = R_values_matsedlabz{4};
    R_SO4_matsedlabzt(:,i) = R_values_matsedlabz{5};
...
%Saving sediments concentrations
R_values_matsedlabzt = { R_oxygen_matsedlabzt, 'Oxygen';
                         R_NO3_matsedlabzt,    'NO3';
                         R_FeOH3_matsedlabzt,  'Fe(OH)3';
                         R_FeOOH_matsedlabzt,  'FeOOH';
                         R_SO4_matsedlabzt,    'SO4'};
...
sediment_results = {O2_matsedlabzt,     'Oxygen (aq)';
                   ...
                   H2S_matsedlabzt,     'Sulfide S(-II) (aq)';
                   HS_matsedlabzt,      'Sulfide S(-I) (aq)'; 
                   ...
                   PO4_matsedlabzt,     'Phosphate PO4 (aq)';
                   ...
                   Ca2_matsedlabzt,     'Calcium Ca2 (aq)';
